...
another with the intention of facilitating integration of data from
...
saved in an independent 'event' table. Event-level data include the contextual
...
Each term in this document is linked with its respective term internationalized resource identifier (IRI) alias (ex., term:eco[eco:protocolNames]). Always use these links to refer to the definition, comments, and examples
...
*Nested datasets* capture more complex survey designs, for example,
...
NOTE: Creating nested event levels may be important to relating the full story a dataset has to tell and to facilitating downstream analysis of the data. The parent-most event levels may  include more general information that applies to all subsequent and lower events. Lower or child-most event levels will include more detailed or explicit information. The lowest or child-most event level would consist of visits to a specific site, implementing a specific protocol, at a unique date/time. However, it is important keep the structure as simple as possible. Do not create unnecessary sub-events.

The structure of nested datasets can take various forms and there is no single correct structure. However, structure is most commonly defined as a function of sampling location, protocol, and date. For example:
...
* *Protocol–locality-date:* This structure establishes sampling protocol at the highest event levels. Subsequent events thsu center location, or survey site, and date. The lowest or child-most event level would consist of visits to a specific site, implementing a specific protocol, at a unique date/time (<<fig2b, Figure 2B>>). +
* *Time-series:* Many survey datasets include multiple visits to a site or series of sites on different dates. These time-series datasets may take either approach and will include, at the child-most event level, an event for each unique site visit (<<fig2c, Figure 2C>>).
