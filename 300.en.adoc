...
much data as possible) from the dataset. Refer to https://rs.gbif.org/extensions.html[GBIF registered
...
structure (<<fig1, Figure 1>>). These are typically simple datasets consisting of:
...
secondarily on protocol (<<fig2a, Figure 2a>>) *locality 🡪 sampling protocol 🡪
occurrence* +
* *Protocol–locality:* or conversely, focusing on protocol at higher
hierarchical levels and secondarily on locality (<<fig2b, Figure 2b>>) .. *protocol
...
Figures <<fig1,1>> and <<fig2a,2>> is particularly useful in exploring and effectively
