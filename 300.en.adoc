...
* *Time-series:* Many survey datasets include multiple visits to a site or series of sites on different dates, but using only a single protocol. These time-series datasets may take either approach and will include, at the child-most event level, an event for each unique site visit (<<fig2c, Figure 2C>>).
...
[[fig2c]]
[caption="Figure 2C. "]
.Simplified example schematic of a nested event dataset structure representing a survey (parent Event, dark purple rectangles) with two distinct survey sites (child-Events, medium purple rectangles). Each survey site is sampled twice using the same protocol (child-child-Events, light purple rectangles). Organismal occurrence information is collected at each of the four site visits and related to appropriate sampling event using the occurrence extension (blue rectangles).
image::img/fig2c.png[]
