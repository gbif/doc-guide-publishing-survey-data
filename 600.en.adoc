...
[NOTE]
.*Using the https://rs.gbif.org/extension/obis/extended_measurement_or_fact_2023-08-28.xml[extended measurements or facts (emof) extension]* 
====
...
- For each unique survey event (term:dwc[dwc:eventID]), populate the relevant extension field(s). 
====
...
- For each unique survey event (term:dwc[dwc:eventID]), populate the relevant extension field(s). Information for a unique term:dwc[dwc:eventID] should require only one row in the table.
====
